 ...
 * delete, maintain:
 *
 *   1. The denormalized `group` field on the student doc
 *      (`gdCollege` | `mlsn` | `skillIndia`), so the app's per-tab queries
 *      keep working no matter which client wrote the document.
 *   2. The `_meta/studentFacets` document — whole-collection admission
 *      years and courses per group, which the app reads for its filter
 *      chips (one small read instead of scanning the collection).
 *
 * The app itself only READS `_meta/studentFacets` (and exact DB counts via
 * aggregate queries); all writes to it happen here.
 *
...
function facetsRef(db) {
  return db.collection("_meta").doc("studentFacets");
}
...
async function addValues(ref, group, year, course) {
  const updates = {};
  if (course.length > 0) {
    updates[`groups.${group}.courses`] = FieldValue.arrayUnion([course]);
  }
  if (year != null) {
    updates[`groups.${group}.years`] = FieldValue.arrayUnion([year]);
  }
...
async function pruneValue(db, ref, group, year, course) {
  ...
  if (year != null) {
    const stillUsed = await db
      ...
      .get();
    if (stillUsed.empty) {
      removals[`groups.${group}.years`] = FieldValue.arrayRemove([year]);
    }
  }
  if (course.length > 0) {
    const stillUsed = await db
      ...
      .get();
    if (stillUsed.empty) {
      removals[`groups.${group}.courses`] = FieldValue.arrayRemove([course]);
    }
  }
...
async function syncStudentDoc(db, beforeSnap, afterSnap) {
  const ref = facetsRef(db);

  // Delete: prune values the removed document was the last user of.
  if (!afterSnap.exists) {
  ...
  // Advertise the new facet values (idempotent unions).
  await addValues(ref, newGroup, newYear, newCourse);

  // Drop old values nothing else still uses.
...
// test database. Stamps `group` on every student doc and rebuilds
// `_meta/studentFacets` wholesale. Safe to re-run.
...
exports.backfillStudentGroups = onCall(
  ...
  async (request) => {
    ...
    for (const name of ["gdCollege", "mlsn", "skillIndia"]) {
      ...
      groups[name] = {years, courses};
    }
    await facetsRef(db).set({groups});
    logger.info(`backfillStudentGroups(${database}): scanned=${scanned} updated=${updated}`);
    return {database, scanned, updated};
  },
);
